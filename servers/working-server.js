...
server.post('/api/students', async (request, reply) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (existingUser) {
        throw new Error(`Já existe um usuário cadastrado com o email ${email} nesta organização.`);
      }
    ...
    if (error.code === 'P2002') {
      ...
      return {
        ...
        message: `Já existe um usuário cadastrado com este email nesta organização.`
      };
    }
...
// Sample organizations endpoint - REMOVED DUPLICATE (already defined at line 58)
...
server.put('/api/billing-plans/:id', async (request, reply) => {
  ...
  try {
    ...
    // Zod schema for billing plan update
    const BillingPlanUpdateSchema = z.object({
      name: z.string().optional(),
      description: z.string().optional(),
      category: z.string().optional(),
      price: z.preprocess(val => val === undefined ? undefined : parseFloat(val), z.number().optional()),
      billingType: z.string().optional(),
      courseId: z.union([z.string(), z.number()]).optional(),
      duration: z.preprocess(val => val === undefined ? undefined : parseInt(val), z.number().optional()),
      classesPerWeek: z.preprocess(val => val === undefined ? undefined : parseInt(val), z.number().optional()),
      maxClasses: z.preprocess(val => val === undefined ? undefined : parseInt(val), z.number().nullable().optional()),
      isUnlimitedAccess: z.boolean().optional(),
      hasPersonalTraining: z.boolean().optional(),
      hasNutrition: z.boolean().optional(),
      accessAllModalities: z.boolean().optional(),
      allowFreeze: z.boolean().optional(),
      freezeMaxDays: z.preprocess(val => val === undefined ? undefined : parseInt(val), z.number().optional()),
      allowInstallments: z.boolean().optional(),
      maxInstallments: z.preprocess(val => val === undefined ? undefined : parseInt(val), z.number().optional()),
      installmentInterestRate: z.preprocess(val => val === undefined ? undefined : parseFloat(val), z.number().nullable().optional()),
      isRecurring: z.boolean().optional(),
      recurringInterval: z.preprocess(val => val === undefined ? undefined : parseInt(val), z.number().nullable().optional()),
      isActive: z.boolean().optional(),
      features: z.any().optional()
    });

    // Validate input
    const parseResult = BillingPlanUpdateSchema.safeParse(body);
...
server.delete('/api/billing-plans/:id', async (request, reply) => {
  try {
    ...
    // Check if plan exists and count active subscriptions
    const existingPlan = await prisma.billingPlan.findUnique({
      where: { id },
      include: {
        _count: {
    ...
    if (!existingPlan) {
      ...
      return {
        success: false,
        error: 'Plan not found',
        message: 'No billing plan found with this ID'
      };
    }
    ...
    if (existingPlan._count.subscriptions > 0) {
      ...
      return {
        ...
        message: 'Cannot delete billing plan with active subscriptions. Please deactivate or transfer subscriptions first.'
      };
    }

    // Soft delete the billing plan
...
server.put('/api/courses/:id', async (request, reply) => {
  try {
    const { id } = request.params;
    const {
      name,
    ...
    const validatedLevel = validLevels.includes(level?.toUpperCase()) ? level.toUpperCase() : existingCourse.level;
    ...
    const course = await prisma.course.update({
      ...
      data: {
        ...
        minAge: ageGroup === '6-12' ? 6 : ageGroup === '13-17' ? 13 : ageGroup === '65+' ? 65 : existingCourse.minAge,
        maxAge: ageGroup === '6-12' ? 12 : ageGroup === '13-17' ? 17 : ageGroup === '65+' ? 99 : existingCourse.maxAge,
        duration: parseInt(duration) || existingCourse.duration,
        totalClasses: parseInt(totalLessons) || existingCourse.totalClasses,
        category: targetAudience || existingCourse.category,
        objectives: Array.isArray(objectives) ? objectives : [objectives || existingCourse.objectives[0]],
        requirements: Array.isArray(evaluationCriteria) ? evaluationCriteria : [evaluationCriteria || existingCourse.requirements[0]]
      },
      include: {
        martialArt: {
          select: {
...
server.delete('/api/courses/:id', async (request, reply) => {
  try {
    ...
    // Check if course has associated classes or students
    if (existingCourse._count.classes > 0 || existingCourse._count.studentCourses > 0) {
      reply.status(400);
      return {
        success: false,
        message: 'Cannot delete course with associated classes or students'
      };
    }
