...
function ensureActiveContainer() {
  // Prefer a high-level container if present
  const container = document.getElementById('activitiesContainer') || document.querySelector('#activities-table')?.closest('[data-module], .module-isolated-container') || document.body;
  if (container && container !== document.body) {
    container.classList.add('module-isolated-container', 'module-active');
    container.dataset.module = 'activities';
...
const ActivitiesModule = {
  ...
  async init() {
    try {
      ...
      // Dispatch app event for module readiness
      window.app?.dispatchEvent?.('module:loaded', { name: 'activities' });
    } catch (err) {
      console.error('Erro ao iniciar módulo de atividades:', err);
      window.app?.handleError?.(err, { module: 'activities', stage: 'init' });
    }
  },
    ...
    try {
      ...
      const response = await activitiesAPI.fetchWithStates('/api/activities', {
        ...
        onError: (error) => {
          ...
          window.app?.handleError?.(error, { module: 'activities', stage: 'fetch' });
      ...
      console.error('Erro ao carregar atividades:', error);
      window.app?.handleError?.(error, { module: 'activities', stage: 'fetch-catch' });
      window.feedback?.showError?.('Erro ao carregar atividades');
      this.showEmptyState('Erro ao carregar atividades');
    }
  },
    ...
    this.activities.forEach(activity => {
      ...
      row.innerHTML = `
        ...
        <td class="activity-actions">
          <button class="btn-action btn-edit" onclick="ActivitiesModule.openEditor('${activity.id}')" title="Editar" aria-label="Editar atividade">
            ✏️
          </button>
          <button class="btn-action btn-delete" onclick="ActivitiesModule.deleteActivity('${activity.id}')" title="Excluir" aria-label="Excluir atividade">
            🗑️
          </button>
        </td>
      `;
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="6" class="empty-state">
          <div class="empty-content data-card-premium">
            <div class="empty-icon">📝</div>
            <h3>${message}</h3>
            ${message.includes('Erro') ? '' : `
              <p>Comece criando sua primeira atividade</p>
              <button class="module-isolated-btn-primary" onclick="ActivitiesModule.openEditor()" aria-label="Nova atividade">
                ➕ Nova Atividade
              </button>
            `}
          </div>
  ...
  openEditor(activityId = null) {
    // Navegar via SPA para garantir renderização no container principal
    const hash = activityId ? `#activity-editor/${encodeURIComponent(activityId)}` : '#activity-editor';
    if (window.router) {
      location.hash = hash;
      window.router.navigateTo('activity-editor');
    } else {
      // Fallback para navegação legacy (não recomendada)
    ...
    try {
      await activitiesAPI.fetchWithStates(`/api/activities/${activityId}`, {
        ...
        onError: (error) => {
          ...
          window.app?.handleError?.(error, { module: 'activities', stage: 'delete' });
          window.feedback?.showError?.('Erro ao excluir atividade');
        }
      });
    } catch (error) {
      console.error('Erro ao excluir atividade:', error);
      window.app?.handleError?.(error, { module: 'activities', stage: 'delete-catch' });
      window.feedback?.showError?.('Erro ao excluir atividade');
    }
  },
...
// Exposição global para uso em HTML e integração com AcademyApp
window.ActivitiesModule = ActivitiesModule;
window.activities = ActivitiesModule;
// Integrar com ModuleLoader se disponível
window.ModuleLoader?.register?.('activities', ActivitiesModule);
