 ...
 * UPDATED: AGENTS.md v2.1 - API Client pattern
 */
...
class AIService {
    ...
    waitForAPIClient() {
        return new Promise((resolve) => {
            ...
            setTimeout(() => {
                clearInterval(checkInterval);
                console.error('AI: API Client não disponível após 10s');
                resolve();
            }, 10000);
        });
    }
     ...
     * UPDATED: Using API Client pattern
    ...
    async fetchFromMCP(tool, parameters = {}) {
        try {
            ...
            const response = await this.moduleAPI.request(`${this.baseUrl}/${tool}`, {
                method: 'POST',
                body: JSON.stringify({
                    tool: tool,
            ...
            window.app?.handleError?.(error, 'AI:fetchFromMCP');
            throw error;
        }
    }
