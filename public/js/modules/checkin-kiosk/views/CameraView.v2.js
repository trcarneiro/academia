...
class CameraView {
    ...
    renderClasses(classes, container) {
        ...
        container.innerHTML = classes.map(turma => {
            ...
            const startTime = schedule.startTime || schedule.time; // e.g., "19:00"
            const duration = schedule.durationMinutes || schedule.duration || 60;
            ...
            } else if (minutesUntilEnd > 0) { // Allow check-in until class ends
                canCheckIn = true;
                const isOngoing = minutesUntilStart <= 0;
                statusText = isOngoing ? 'Em Andamento' : 'Check-in Aberto';
