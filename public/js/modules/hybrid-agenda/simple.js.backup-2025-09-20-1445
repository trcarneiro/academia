/**
 * Hybrid Agenda Module - Legacy Version (DEPRECATED)
 * 
 * ⚠️ ESTE ARQUIVO ESTÁ DEPRECATED ⚠️
 * 
 * STATUS: COMPLETAMENTE MIGRADO PARA index.js
 * 
 * ✅ MIGRADO PARA index.js (Padrão AGENTS.md):
 *    - ✅ initHybridAgendaModule() → Integração SPA completa
 *    - ✅ showUnifiedModal() → Interface unificada create/edit
 *    - ✅ handleAgendaItemClick() → Navegação para edição
 *    - ✅ showCreateModal() → Redirecionado para showUnifiedModal()
 *    - ✅ showEditModal() → Integrado no showUnifiedModal()
 *    - ✅ deleteAgendaItem() → Migrado com confirmação
 *    - ✅ Timezone fix aplicado
 *    - ✅ Premium UI components (module-header-premium, stat-card-enhanced)
 *    - ✅ Integração com AcademyApp
 *    - ✅ Exportações window.* para compatibilidade
 * 
 * 🎯 IMPLEMENTAÇÃO ATIVA: /public/js/modules/hybrid-agenda/index.js
 * 
 * 📋 PRÓXIMOS PASSOS:
 *    - Testar funcionalidades migradas
 *    - Remover este arquivo após validação completa
 *    - Eventualmente substituir por stub de compatibilidade
 * 
 * Data da Migração: 20/09/2025
 * Status: DEPRECATED - Use index.js
 */

console.warn('⚠️ HYBRID AGENDA SIMPLE.JS IS DEPRECATED');
console.warn('✅ All functionality migrated to index.js (AGENTS.md compliant)');
console.warn('🎯 Use: window.initHybridAgendaModule() from index.js');

// Redirect all functions to the new implementation
if (typeof window !== 'undefined') {
    
    // Compatibility layer - redirect to index.js implementation
    window.initHybridAgendaModule = window.initHybridAgendaModule || function(...args) {
        console.warn('⚠️ Loading index.js implementation...');
        return import('./index.js').then(module => {
            return window.initHybridAgendaModule(...args);
        });
    };
    
    // Log deprecation warning
    setTimeout(() => {
        console.warn('%c⚠️ HYBRID AGENDA DEPRECATION NOTICE', 'color: orange; font-weight: bold;');
        console.warn('simple.js is deprecated. All functionality is now in index.js');
        console.warn('Please update your code to use the AGENTS.md compliant version.');
    }, 1000);
}
...
async function initializeAPI() {
    console.log('🔧 Checking for API client...');
    console.log('🔧 window.createModuleAPI:', typeof window.createModuleAPI);
    
    if (!window.createModuleAPI) {
        console.log('❌ API client not found, waiting...');
    ...
    console.log('✅ Module API created:', moduleAPI);
}

// Main initialization function for SPA integration
async function initHybridAgendaModule(container, route) {
    try {
        console.log('🏛️ Initializing Hybrid Agenda Module (Simplified)...', route);
        ...
        container.innerHTML = `
            <div class="error-state">
                ...
                <p>Não foi possível inicializar o módulo: ${error.message}</p>
...
function setupEventListeners() {
    document.addEventListener('click', (e) => {
        ...
        if (e.target.matches('.debug-list-btn')) {
            console.log('🔧 Switching to list view for debugging');
        ...
        if (e.target.matches('.agenda-item') || e.target.closest('.agenda-item')) {
            ...
            if (item) {
                console.log('🎯 Agenda item clicked:', item.dataset.id, item.dataset.type);
                handleAgendaItemClick(item.dataset.id, item.dataset.type);
            } else {
                console.log('⚠️ No agenda item found in target:', e.target);
            }
        }
        
        // Debug: Log all clicks to see what's being clicked
        console.log('🖱️ Click event:', e.target, e.target.className);
    });
        ...
        const response = await moduleAPI.fetchWithStates('/api/hybrid-agenda', {
            ...
            onSuccess: (response) => {
                ...
                if (response && response.items && Array.isArray(response.items)) {
                    ...
                    console.log('✅ Agenda data loaded from API:', agendaData.length, 'items');
                } else {
                ...
                renderCurrentView();
                console.log('🔧 renderCurrentView() call completed');
                // Debug: Check if agenda items exist
                setTimeout(() => {
                    const items = document.querySelectorAll('.agenda-item');
                    console.log('🔍 Debug: Found', items.length, 'agenda items in DOM');
                    items.forEach((item, index) => {
                        console.log(`🔍 Item ${index}:`, item.dataset.id, item.className);
                    });
                }, 100);
            },
            onEmpty: () => {
        ...
        const response = await moduleAPI.fetchWithStates('/api/hybrid-agenda', {
            onSuccess: (response) => {
                ...
                if (response && response.summary) {
                    ...
                    console.warn('⚠️ No summary in API response, using defaults');
...
function renderCurrentView() {
    ...
    console.log('🔧 renderCurrentView - contentContainer:', contentContainer);
    if (!contentContainer) return;
    
    if (currentView === 'calendar') {
        console.log('🔧 renderCurrentView - Calling renderCalendarView');
...
function renderCalendarView(container) {
    console.log('🔧 renderCalendarView called with container:', container);
    console.log('🔧 renderCalendarView - agendaData:', agendaData.length, 'items');
    
    const calendarHtml = `
        <div class="calendar-view">
            <div class="calendar-header">
                <h3>Calendário Semanal</h3>
                <p>Visualização de turmas e sessões de personal training</p>
            </div>
    ...
    console.log('🔧 renderCalendarView - Setting innerHTML');
    container.innerHTML = calendarHtml;
    console.log('🔧 renderCalendarView - innerHTML set, adding click listeners');
    ...
    const listHtml = `
        <div class="list-view">
            <div class="list-header">
                ...
                <p>Todos os agendamentos do período (${agendaData.length} ${agendaData.length === 1 ? 'item' : 'itens'})</p>
            </div>
...
function renderWeekDays() {
    ...
    console.log('🔧 renderWeekDays - agendaData:', agendaData);
    console.log('🔧 renderWeekDays - currentDate:', currentDate);
    console.log('🔧 renderWeekDays - weekStart:', weekStart);
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const dayItems = agendaData.filter(item => {
            ...
            console.log(`🔧 Item ${item.id} (${item.title}): itemDate=${itemDate.toDateString()}, day=${day.toDateString()}, match=${dayMatch}`);
            return dayMatch;
        });
        
        console.log(`🔧 Day ${day.toDateString()}: ${dayItems.length} items`);
    ...
    console.log('🔧 renderWeekDays - Total items rendered:', totalItemsRendered);
    return days.join('');
}
...
function renderDayItem(item) {
    ...
    const recurrenceIcon = item.isRecurring ? '🔄' : '';
        ...
        <div class="data-card-premium agenda-item ${typeClass}" 
            ...
            <div class="agenda-item-content">
                ...
                <div class="agenda-item-details">
                    ...
                    <div class="detail-group">
                        <span class="detail-icon">👨‍🏫</span>
                        <span class="detail-text">${item.instructor?.name || 'Instrutor não definido'}</span>
                    </div>
                    
                    <div class="detail-group">
                        <span class="detail-icon">📍</span>
                        <span class="detail-text">${item.trainingArea?.name || 'Local não definido'}</span>
                    </div>
...
async function renderDetailsPage(container, itemId) {
    ...
    if (!item) {
        container.innerHTML = `
            <div class="error-state">
                ...
                <button class="btn-primary" onclick="window.location.hash = '#/hybrid-agenda'">← Voltar para Agenda</button>
            </div>
        `;
        return;
    }
    ...
    const typeInfo = item.type === 'PERSONAL_SESSION' 
        ? { icon: '👤', title: 'Personal Training', color: '#764ba2' }
        : { icon: '🥋', title: 'Aula Coletiva', color: '#667eea' };
                        ...
                        <div class="details-header" style="background: linear-gradient(135deg, ${typeInfo.color}, ${typeInfo.color}dd)">
                            <div class="details-title">
                                ...
                                <div>
                                    <h2>${item.title}</h2>
                                    <p>${typeInfo.title}</p>
                                    ${item.isRecurring ? '<span class="recurrence-badge">🔄 Recorrente</span>' : ''}
                                </div>
                            </div>
                ...
                <div class="details-content">
                    <div class="details-grid">
                        ...
                        <div class="detail-item">
                            ...
                            <span class="detail-value">${item.instructor?.name || 'Não definido'}</span>
                        </div>
                        
                        <div class="detail-item">
                            <span class="detail-label">📍 Local</span>
                            <span class="detail-value">${item.trainingArea?.name || item.unit?.name || 'Não definido'}</span>
                        </div>
...
function editAgendaItem(itemId) {
    ...
    if (!item) {
        window.app.handleError(new Error('Item não encontrado'), 'Edit Agenda Item');
        return;
    }
        ...
        <div class="scheduling-menu-overlay" onclick="closeCreateMenu(event)">
            <div class="scheduling-menu">
                <div class="scheduling-menu-header">
                    <h4>🗓️ Agendar para ${formatDateForDisplay(date)} às ${time}</h4>
                </div>
                <div class="scheduling-options">
                    ...
                    <button class="scheduling-option collective-class" 
                        ...
                        <div class="option-content">
                            ...
                            <div class="option-description">Turma com múltiplos alunos</div>
                        </div>
                    </button>
                </div>
        ...
        <div class="scheduling-form-overlay" onclick="closeSchedulingForm(event)">
            <div class="scheduling-form">
                <div class="form-header">
                    <h3>👤 Novo Personal Training</h3>
                    <p>📅 ${formatDateForDisplay(date)} às ${time}</p>
                    <button class="close-form-btn" onclick="closeSchedulingForm()">✕</button>
                </div>
                
                <form id="personal-session-form" onsubmit="submitPersonalSession(event)">
                    <div class="form-group">
                        <label for="session-title">Título da Sessão *</label>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="session-description" name="description" rows="3"
                                  placeholder="Objetivos da sessão, foco específico..."></textarea>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label for="session-start-time">Início *</label>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label for="session-recurring">Sessão Recorrente</label>
                        ...
                        <div class="form-group">
                            <label for="session-recurrence-end">Recorrência até (opcional)</label>
                ...
                <form id="collective-class-form" onsubmit="submitCollectiveClass(event)">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="class-description" name="description" rows="3"
                                  placeholder="Técnicas a serem praticadas, nível da turma..."></textarea>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label for="class-start-time">Início *</label>
                            <input type="time" id="class-start-time" name="startTime" value="${time}" required>
                        </div>
                        <div class="form-group">
                            <label for="class-duration">Duração (min) *</label>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label for="class-max-students">Máximo de Alunos *</label>
...
async function loadFormData() {
    try {
        ...
        showErrorMessage('Erro ao carregar dados do formulário');
    }
}
...
async function loadTrainingAreas() {
    try {
        const response = await moduleAPI.fetchWithStates('/api/training-areas', {
            method: 'GET',
            onSuccess: (data) => {
                console.log('✅ Training areas loaded:', data);
...
function populateTrainingAreaSelects(areas) {
    ...
    selects.forEach(select => {
        select.innerHTML = '<option value="">Nenhum local específico</option>';
        
        if (areas.length === 0) {
            return; // Permite criar sem área específica
        }
...
function handleItemClick(event) {
    ...
    console.log('🎯 Agenda item clicked via addItemClickListeners:', itemId, itemType);
    
    if (itemId) {
        handleAgendaItemClick(itemId, itemType);
    } else {
        console.error('❌ Agenda item ID not found:', itemId);
    }
}
...
async function submitPersonalSession(event) {
    ...
    try {
        ...
        const response = await moduleAPI.fetchWithStates('/api/hybrid-agenda', {
            ...
            body: sessionData,
            onSuccess: (result) => {
                console.log('✅ Personal session created:', result);
                closeSchedulingForm();
            ...
            onError: (error) => {
                ...
                if (error.message?.includes('instructorId')) {
                    errorMessage += ': Instrutor é obrigatório';
...
async function submitCollectiveClass(event) {
    ...
    try {
        ...
        const response = await moduleAPI.fetchWithStates('/api/hybrid-agenda', {
            ...
            body: classData,
            onSuccess: (result) => {
                console.log('✅ Collective class created:', result);
                closeSchedulingForm();
                showSuccessMessage('Aula coletiva agendada com sucesso!');
                loadAgendaData(); // Reload data
            },
            onError: (error) => {
                console.error('❌ Failed to create collective class:', error);
                let errorMessage = 'Erro ao agendar Aula Coletiva';
                
                if (error.message?.includes('trainingAreaId')) {
                    errorMessage += ': Área de treino é obrigatória';
                } else if (error.message?.includes('maxParticipants')) {
                    errorMessage += ': Número máximo de participantes inválido';
                } else if (error.message?.includes('validation')) {
                    errorMessage += ': Dados inválidos';
                } else if (error.message) {
                    errorMessage += ': ' + error.message;
...
async function renderEditPage(container, itemId) {
    console.log(`🔧 Rendering edit page for item: ${itemId}`);
    ...
    const pageIcon = isPersonalTraining ? '👤' : '🥋';
        ...
        <div class="edit-form-container">
            <div class="form-card-premium">
                ...
                <form id="edit-agenda-form" class="agenda-form-premium">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label for="edit-max-students">Máximo de Alunos *</label>
                    ...
                    <div id="edit-recurrence-options" class="recurrence-options" 
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label for="edit-recurrence-pattern">Frequência</label>
...
async function updateAgendaItem(itemId, originalData) {
    try {
        ...
        showErrorMessage('❌ Erro ao atualizar agendamento: ' + (error.message || 'Erro desconhecido'));
    }
}
...
async function createNewAgendaItem(type) {
    try {
        ...
        showErrorMessage('❌ Erro ao criar agendamento: ' + (error.message || 'Erro desconhecido'));
    }
}
...
async function deleteAgendaItem(itemId) {
    ...
    try {
        const response = await moduleAPI.fetchWithStates(`/api/hybrid-agenda/${itemId}`, {
            method: 'DELETE',
            onSuccess: (result) => {
                console.log('✅ Agendamento deleted:', result);
                closeEditModal(); // Close modal if open
                showSuccessMessage('Agendamento excluído com sucesso!');
                loadAgendaData(); // Reload data
            },
            onError: (error) => {
                console.error('❌ Failed to delete agendamento:', error);
            ...
            <div class="modal-body">
                <form id="create-agenda-form">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label for="create-startTime">Horário *</label>
                            <input type="time" id="create-startTime" name="startTime" required>
                        </div>
                        <div class="form-group">
                            <label for="create-duration">Duração (min) *</label>
                    ...
                    <div class="form-group">
                        <label for="create-trainingAreaId">Área de Treino</label>
                    ...
                    <div id="create-turma-fields" style="display: none;">
                        <div class="form-group">
                            <label for="create-maxStudents">Máximo de Alunos *</label>
                            <input type="number" id="create-maxStudents" name="maxStudents" min="1" max="50" value="10">
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label for="create-recurring">Recorrência</label>
                        <select id="create-recurring" name="recurring">
                            <option value="false">Agendamento único</option>
                            <option value="weekly">Semanal</option>
                            <option value="monthly">Mensal</option>
                        </select>
                    </div>
...
async function loadCreateFormData() {
    try {
        ...
        const instructorsResponse = await moduleAPI.fetchWithStates('/api/instructors', {
            onSuccess: (response) => {
                console.log('✅ Create Instructors API response:', response);
                ...
                if (instructorSelect && instructors.length > 0) {
                    ...
                    console.log('✅ Added', instructors.length, 'instructors to create select');
                }
            }
        });
        ...
        const unitsResponse = await moduleAPI.fetchWithStates('/api/units', {
            onSuccess: (response) => {
                console.log('✅ Create Units API response:', response);
                ...
                if (unitSelect) {
                    ...
                    if (units.length > 0) {
                        ...
                        console.log('✅ Added', units.length, 'units to create select');
                    }
                } else {
                    console.error('❌ Unit select element not found: create-unitId');
                }
            }
        });
        ...
        console.log('✅ Create form data loading completed');
    } catch (error) {
...
async function handleCreateFormSubmit(event) {
    ...
    console.log('🔧 Create form data captured:', data);
    ...
    try {
        ...
        console.log('✅ Agendamento created:', result);
        ...
        console.error('❌ Failed to create agendamento:', error);
...
// ==================== MIGRATED TO index.js ====================
// The following function has been migrated to HybridAgendaModule.showUnifiedModal()
// following AGENTS.md standards (full-screen pages instead of modals)

/*
// DEPRECATED: Migrated to index.js
...
async function showUnifiedCreateModal() {
    ...
    console.log('🔧 Loading unified form data...');
...
async function loadUnifiedFormData() {
    console.log('🔧 Starting loadUnifiedFormData...');
    ...
    if (!instructorSelect) {
        console.error('❌ CRITICAL: instructorId select not found!');
        // List all select elements to debug
        const allSelects = document.querySelectorAll('select');
        console.log('🔧 All select elements in modal:', Array.from(allSelects).map(s => ({ id: s.id, classes: s.className })));
        return;
    }
    ...
    try {
        ...
        if (result.success && result.data && result.data.length > 0) {
            ...
            console.log('✅ Successfully populated instructor select with', result.data.length, 'instructors');
            console.log('🔧 Final instructor options count:', instructorSelect.options.length);
        } else {
            console.error('❌ No instructors data received or empty array');
        }
    } catch (error) {
        console.error('❌ Failed to load instructors:', error);
    }
    ...
    if (unitSelect) {
        try {
            ...
            if (unitsResult.success && unitsResult.data) {
                ...
                console.log('✅ Populated units:', unitsResult.data.length);
                ...
                unitSelect.addEventListener('change', async function() {
                    ...
                    console.log('🔧 Create modal - Unit changed to:', selectedUnitId);
            ...
            console.log('⚠️ Units not available:', error.message);
        }
    }
    ...
    if (trainingAreaSelect && !unitSelect) {
        ...
        try {
            ...
            if (areasResult.success && areasResult.data) {
                trainingAreaSelect.innerHTML = '<option value="">Selecione uma área</option>';
                ...
                console.log('✅ Populated training areas (all):', areasResult.data.length);
            }
        } catch (error) {
            console.log('⚠️ Training areas not available:', error.message);
        }
    }
    
    console.log('🔧 loadUnifiedFormData completed');
}
...
async function loadCreateTrainingAreasByUnit(unitId) {
    ...
    console.log('🔧 Create modal - Loading training areas for unit:', unitId);
    
    try {
        const areasResponse = await fetch(`/api/training-areas?unitId=${unitId}`);
        const areasResult = await areasResponse.json();
        
        console.log('✅ Create modal - Filtered Training Areas API response:', areasResult);
        ...
        if (areasResult.success && areasResult.data) {
            ...
            if (areas.length > 0) {
                areas.forEach(area => {
                    ...
                    console.log(`🔧 Create modal - Added area: ${area.name}`);
                });
                console.log('✅ Create modal - Added', areas.length, 'filtered training areas');
            } else {
                areaSelect.innerHTML = '<option value="">Nenhuma área encontrada para esta unidade</option>';
            }
        } else {
        ...
        console.error('❌ Create modal - Error loading training areas for unit:', error);
        areaSelect.innerHTML = '<option value="">Erro ao carregar áreas</option>';
        areaSelect.disabled = true;
    }
}
...
async function handleUnifiedFormSubmit(event) {
    ...
    console.log('🔧 unitId value:', data.unitId);
    console.log('🔧 unitId type:', typeof data.unitId);
    ...
    console.log('🔧 baseData.unitId:', baseData.unitId);
    ...
    try {
        ...
        const response = await moduleAPI.fetchWithStates(`/api/hybrid-agenda/${itemId}`, {
            ...
            onError: (error) => {
                ...
                showErrorMessage('Erro ao carregar agendamento para edição');
            }
        });
    } catch (error) {
        console.error('Error loading item for edit:', error);
            ...
            <div class="modal-body">
                <form id="edit-agenda-form">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label for="edit-startTime">Horário *</label>
                    ...
                    <div class="form-group">
                        <label for="edit-trainingAreaId">Área de Treino</label>
                    ...
                    ${item.type === 'TURMA' ? `
                    <div class="form-group">
                        <label for="edit-maxStudents">Máximo de Alunos *</label>
...
async function loadEditFormData(item) {
    try {
        console.log('🔄 Loading edit form data for item:', item);
        console.log('🔧 Item IDs for comparison - instructorId:', item.instructorId, 'unitId:', item.unitId, 'trainingAreaId:', item.trainingAreaId);
        
        // Load instructors
        const instructorsResponse = await moduleAPI.fetchWithStates('/api/instructors', {
            onSuccess: (response) => {
                console.log('✅ Edit Instructors API response:', response);
                ...
                const instructors = Array.isArray(response) ? response : (response.data || []);
                console.log('🔧 Item instructorId for comparison:', item.instructorId);
                if (instructorSelect && instructors.length > 0) {
                    ...
                    instructors.forEach(instructor => {
                        ...
                        console.log(`🔧 Instructor ${instructor.name}: userId=${instructor.userId}, selected=${isSelected}`);
                        instructorSelect.appendChild(option);
                    });
                    console.log('✅ Added', instructors.length, 'instructors to edit select');
                } else {
                    console.warn('⚠️ No instructors found for edit or select element missing');
                }
            }
        });
        ...
        const unitsResponse = await moduleAPI.fetchWithStates('/api/units', {
            onSuccess: (response) => {
                console.log('✅ Edit Units API response:', response);
                const unitSelect = document.getElementById('edit-unitId');
                console.log('🔧 Unit select element:', unitSelect);
                console.log('🔧 Unit select innerHTML before:', unitSelect ? unitSelect.innerHTML : 'ELEMENT NOT FOUND');
                // fetchWithStates already extracts data, so response is the data array directly
                const units = Array.isArray(response) ? response : [];
                console.log('🔧 Units data:', units);
                console.log('🔧 Item unitId for comparison:', item.unitId);
                if (unitSelect) {
                    // Clear existing options (except default)
                    unitSelect.innerHTML = '<option value="">Selecione uma unidade</option>';
                    if (units.length > 0) {
                        units.forEach(unit => {
                            ...
                            option.selected = isSelected;
                            console.log(`🔧 Unit ${unit.name}: id=${unit.id}, selected=${isSelected}`);
                            unitSelect.appendChild(option);
                        });
                        console.log('✅ Added', units.length, 'units to edit select');
                    ...
                    console.error('❌ Unit select element not found: edit-unitId');
                }
            }
        });
...
async function loadTrainingAreasByUnit(unitId, targetSelectId = 'edit-trainingAreaId') {
    ...
    if (!areaSelect) {
        console.error(`❌ Area select element not found: ${targetSelectId}`);
        return;
    }
    ...
    console.log('🔧 Loading training areas for unit:', unitId, 'targetSelect:', targetSelectId);
    
    try {
        const areasResponse = await moduleAPI.fetchWithStates(`/api/training-areas?unitId=${unitId}`, {
            onSuccess: (response) => {
                console.log('✅ Filtered Training Areas API response:', response);
                const areas = Array.isArray(response) ? response : [];
                console.log('🔧 Filtered areas data:', areas);
                ...
                if (areas.length > 0) {
                    areas.forEach(area => {
                        ...
                        if (targetSelectId.includes('edit')) {
                            ...
                            option.selected = isSelected;
                            console.log(`🔧 Filtered Area ${area.name}: id=${area.id}, selected=${isSelected}`);
                        }
                        areaSelect.appendChild(option);
                    });
                    console.log('✅ Added', areas.length, 'filtered training areas to select');
                } else {
                    areaSelect.innerHTML = '<option value="">Nenhuma área encontrada para esta unidade</option>';
                }
            },
            onError: (error) => {
                console.error('❌ Error loading training areas for unit:', error);
                areaSelect.innerHTML = '<option value="">Erro ao carregar áreas</option>';
                areaSelect.disabled = true;
            }
        });
    } catch (error) {
        console.error('❌ Error in loadTrainingAreasByUnit:', error);
        areaSelect.innerHTML = '<option value="">Erro ao carregar áreas</option>';
        areaSelect.disabled = true;
    }
}
...
async function handleEditFormSubmit(event) {
    ...
    console.log('🔧 Specific unitId value:', data.unitId);
    ...
    try {
        ...
        console.log('🔧 Update URL:', `/api/hybrid-agenda/${itemId}`);
        ...
        const result = await response.json();
        console.log('✅ Agendamento updated:', result);
        ...
        console.error('❌ Failed to update agendamento:', error);
...
window.handleAgendaItemClick = handleAgendaItemClick;
// DEPRECATED: Use hybridAgenda.showUnifiedModal() from index.js instead
...
console.log('✅ Hybrid Agenda module (simplified) loaded');
console.log('🔧 initHybridAgendaModule exported:', typeof window.initHybridAgendaModule);