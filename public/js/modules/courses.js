(function () {
    ...
    async function initializeCoursesModule() {
        ...
        // Fix for SPA Navigation: Check if we need to re-init
        const tempContainer = document.getElementById('coursesContainer');
        if (tempContainer) {
            const isDomActive = tempContainer.getAttribute('data-active') === 'true';
            if (isInitialized && !isDomActive) {
                console.log('⚠️ DOM reset detected - forcing re-initialization');
                isInitialized = false;
            }
        }
    ...
    async function loadInitialData() {
        ...
        try {
            let result;
            if (coursesAPI) {
                console.log('🌐 Using CoursesAPI Client for request');
                try {
                    result = await coursesAPI.request('/api/courses');
                } catch (apiError) {
    ...
    window.deleteCourse = async function (courseId, courseName) {
        ...
        try {
            if (coursesAPI) {
                console.log('🌐 Using CoursesAPI for DELETE');
                const result = await coursesAPI.request(`/api/courses/${courseId}`, {
                    method: 'DELETE'
                });
