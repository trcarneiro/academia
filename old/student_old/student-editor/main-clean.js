...
class StudentEditor {
    ...
    extractStudentId() {
        ...
        // Se não encontrar na URL, tenta no localStorage (navegação interna)
        if (!studentId) {
            try {
                ...
                // Tentar a chave nova usada pelo sistema de navegação
                ...
                if (storedData) {
                    ...
                    if (navData.mode === 'edit' && navData.studentId) {
                        ...
                        console.log(`📋 ID do estudante recuperado do localStorage (studentEditorMode): ${studentId}`);
                    }
                }
                
                // Fallback para chave antiga se não encontrar
                if (!studentId) {
        ...
        if (!this.currentStudentId) {
            console.error('❌ ID do estudante não encontrado na URL nem no localStorage');
            this.showError('ID do estudante não encontrado. Redirecionando...');
            setTimeout(() => {
        ...
        console.log(`📋 ID do estudante: ${this.currentStudentId}`);
        return true;
    }
    ...
    updateHeader() {
        ...
        if (nameElement) nameElement.textContent = this.studentData.name || 'Nome não informado';
        if (idElement) idElement.textContent = `ID: ${this.studentData.id || '--'}`;
        if (categoryElement) categoryElement.textContent = `Status: ${this.studentData.status || 'Indefinido'}`;
    }

    async saveAllChanges() {
        if (!this.studentData || !this.currentStudentId) {
            this.showError('Nenhum dado para salvar');
            return;
        }
