(function() {
    ...
    window.disposeStudentsModule = () => { disposed = true; };
    ...
    async function loadPlansForFilter() {
        ...
        try {
            ...
            if (data.success && data.data) {
                ...
                console.warn('⚠️ Failed to load plans:', data.error || 'Unknown error');
                // Use fallback/mock data se a API falhar
                const fallbackPlans = [
                    { id: 'basic', name: 'Plano Básico', isActive: true },
                    { id: 'premium', name: 'Plano Premium', isActive: true }
                ];
                populatePlanFilter(fallbackPlans);
                console.log('✅ Using fallback plans:', fallbackPlans.length);
            }
        } catch (error) {
            console.error('❌ Error loading plans for filter:', error);
            // Use fallback em caso de erro completo
            const fallbackPlans = [
                { id: 'basic', name: 'Plano Básico', isActive: true },
                { id: 'premium', name: 'Plano Premium', isActive: true }
            ];
            populatePlanFilter(fallbackPlans);
            console.log('✅ Using emergency fallback plans');
        }
    }
    ...
    // **NOVA FUNÇÃO: Adicionar Novo Aluno - Seguindo CLAUDE.md**
    function addNewStudent() {
        console.log('➕ Navegando para tela de adicionar estudante...');
        
        // Salvar modo no localStorage conforme especificado
        localStorage.setItem('studentEditorMode', JSON.stringify({
            mode: 'create',
            studentId: null,
            timestamp: Date.now()
        ...
        // Navegar para student-editor (Uma Ação = Uma Tela)
    ...
    window.studentsModule = {
        ...
        loadProgressTab,
        loadInsightsTab,
        addNewStudent, // ✅ Nova função exportada
        STUDENT_TABS
    };
