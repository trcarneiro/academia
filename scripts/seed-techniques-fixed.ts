 ...
 *
 * Execução:
 *  npx ts-node scripts/seed-techniques-fixed.ts
 * ou
 *  ts-node scripts/seed-techniques-fixed.ts
 *
 * Ajuste crítico: coluna reservada "references" está devidamente escapada com aspas duplas
 * na instrução SQL (INSERT e ON CONFLICT).
 */
...
async function main() {
  console.log(`[seed-techniques-fixed] Starting at ${nowIso()}`);
  ...
  console.log(`[seed-techniques-fixed] Completed at ${nowIso()}`);
}
...
main()
  ...
  .catch(async (e) => {
    console.error('[seed-techniques-fixed] Error', e);
    await prisma.$disconnect();
    process.exit(1);
  });