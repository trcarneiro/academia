...
class FrequencyStatsService {
   ...
   */
  static async getChartsData(organizationId) {
    try {
   ...
   * @private
   */
  static async getWeeklyStats(organizationId, startDate) {
   ...
   * @private
   */
  static async getTopStudents(organizationId, startDate) {
    const checkins = await prisma.turmaAttendance.findMany({
      where: {
        turma: { organizationId },
        createdAt: { gte: startDate },
   ...
   * @private
   */
  static async getClassesByAttendance(organizationId, startDate) {
   ...
   */
  static async getStudentsMissingWithActivePlans(organizationId, daysThreshold = 7) {
    try {
   ...
   */
  static async getLessonsHistory(organizationId, options = {}) {
    try {
      const {
        page = 1,
        pageSize = 20,
...
module.exports = { FrequencyStatsService };
