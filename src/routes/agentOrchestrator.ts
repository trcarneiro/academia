 ...
 * API para criar, gerenciar e executar agentes autï¿½nomos
 */
...
export async function agentOrchestratorRoutes(fastify: FastifyInstance) {
     ...
     * Sugerir agentes para criar baseado no negï¿½cio
     * ? RETORNA: Agentes existentes + Novas sugestï¿½es da IA
     */
    fastify.post('/orchestrator/suggest', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            ...
            // ?? BUSCAR AGENTES EXISTENTES (jï¿½ criados)
            ...
            const fallbackSuggestions = [
                { name: 'Assistente Administrativo', type: 'financeiro', description: 'Monitora planos, pagamentos e inscriï¿½ï¿½es; sugere aï¿½ï¿½es e relatï¿½rios.', tools: ['database', 'reports', 'notifications'] },
                { name: 'Agente Pedagï¿½gico', type: 'pedagogico', description: 'Analisa cursos e planos de aula; sugere melhorias baseadas em dados.', tools: ['lesson_plans', 'courses', 'activity_stats'] },
                { name: 'Agente de Marketing', type: 'marketing', description: 'Analisa leads e campanhas; propï¿½e prï¿½ximas aï¿½ï¿½es comerciais.', tools: ['crm', 'google_ads'] }
            ];

            // ?? COMBINAR: Agentes existentes + Sugestï¿½es novas
            const existingFormatted = existingAgents.map(agent => ({
                id: agent.id,
                name: agent.name,
                type: agent.specialization,
                description: agent.description || 'Agente jï¿½ criado',
            ...
            const fb = [
                { name: 'Assistente Administrativo', type: 'financeiro', description: 'Acompanha finanï¿½as e inscriï¿½ï¿½es.', tools: ['database', 'reports'], status: 'suggested' }
            ];
    ...
    fastify.post('/orchestrator/create', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            ...
            // Verificar permissï¿½o de criaï¿½ï¿½o
     ...
     * Obter histï¿½rico de execuï¿½ï¿½es de um agente
     */
